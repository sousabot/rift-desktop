...
export default function Pros() {
  ...
  useEffect(() => {
    if (!api?.list) {
      setPayload({ players: [], countries: [], leagues: [], status: 'need-app' });
      return undefined;
    }
    let alive = true;
  ...
  return (
    <div className="pr-page">
      ...
      {payload.status === 'need-app' ? (
        <p className="pr-empty">{t('pros.needApp')}</p>
      ) : null}
      ...
      {payload.status === 'need-app' || (!payload.countries.length && payload.status !== 'ready') ? null : (
