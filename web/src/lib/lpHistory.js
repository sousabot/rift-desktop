...
export function formatLpDelta(value, estimated = false) {
  if (!isPlausibleLpDelta(value)) return '';
  const rounded = Math.round(Number(value));
  const signed = rounded > 0 ? `+${rounded}` : String(rounded);
  return `${estimated ? '~' : ''}${signed} LP`;
}

/** Typical ranked swing when Riot/U.GG did not give a measured delta. */
const TYPICAL_LP = {
  IRON: [20, 20],
  BRONZE: [20, 20],
  SILVER: [21, 20],
...
export function fillEstimatedLp(games, rankTier) {
  const pair = TYPICAL_LP[String(rankTier || '').toUpperCase()];
  if (!pair) return games;
  return (games || []).map((g) => {
    if (isPlausibleLpDelta(g.lpDelta) && !g.lpDeltaEst) return g;
    const q = Number(g.queueId);
    if (q !== 420 && q !== 440) return g;
    const mins = Number(g.durationMin) || 0;
    if (mins > 0 && mins < 5) return g;
    if (g.win == null) return g;
    return {
      ...g,
      lpDelta: g.win ? pair[0] : -pair[1],
      lpDeltaEst: true,
    };
  });
}
...
export function applyTrackedLp(games, lpByNumericId, riotId, mode) {
  ...
  return games.map((g) => {
    ...
    const n = Number(id ? map[id] : map[String(g.matchId)] ?? null);
    if (!deltaFitsGame(g, n)) return g;
    rememberLpDelta(riotId, mode, g.matchId, n);
    return { ...g, lpDelta: Math.round(n), lpDeltaEst: false };
  });
}
...
export function syncMatchLp({ riotId, mode, lp, tier, division, games, queueId } = {}) {
  ...
  const map = readLpMap(riotId, mode);
  const measured = list.map((g) => ({
    ...g,
    lpDelta: map[g.matchId] != null ? map[g.matchId] : (g.lpDelta ?? null),
    lpDeltaEst: map[g.matchId] != null ? false : g.lpDeltaEst,
  }));
  return fillEstimatedLp(measured, tier);
}
