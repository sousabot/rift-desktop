/** ARAM tier list — Lolalytics SSR (mega API ignores ARAM queue). */
...
function cachePath() {
  ...
  return path.join(dir, 'aram-cache.json');
}
...
async function httpGetText(url) {
  ...
  if (!res.ok) throw new Error(`lolalytics aram ${res.status}`);
  return res.text();
}
...
function extractAramPayload(html) {
  const match = html.match(/<script type="qwik\/json">([\s\S]*?)<\/script>/);
  if (!match) throw new Error('aram page missing data');
  let parsed;
  try {
    parsed = JSON.parse(match[1]);
  } catch {
    throw new Error('aram page data corrupt');
  }
  ...
  for (const o of objs) {
    ...
    const isAram = queue === QUEUE_ARAM || /aram/i.test(cache);
    if (!isAram) continue;
    if (!best || count > Object.keys(best.cid).length) best = resolved;
  }
  if (!best) throw new Error('aram cid missing');
  return best;
}
...
function pageUrl({ rank, region, patch }) {
  ...
  return `https://lolalytics.com/lol/tierlist/aram/?${q.toString()}`;
}

async function fetchAram({ platform = 'euw1', rank = 'emerald_plus', regionOverride = '' } = {}) {
  ...
  const payload = extractAramPayload(html);
  const rows = rowsFromPayload(payload, names);
  if (!rows.length) throw new Error('aram empty');
  return {
    ok: true,
    platform,
    rank,
    region,
    patch: patch || 'live',
    queue: Number(payload.queue) || QUEUE_ARAM,
    matches: Number(payload.analysed) || 0,
    analysed: Number(payload.analysed) || 0,
    avgWr: payload.avgWr != null ? Number(payload.avgWr) : null,
    builtAt: Date.now(),
    source: 'lolalytics-aram',
    rows,
  };
}

async function getAram({ platform = 'euw1', rank = 'emerald_plus', region = 'all', force = false } = {}) {
  ...
  const job = (async () => {
    try {
      let data;
      try {
        data = await fetchAram({ platform: plat, rank: rk, regionOverride: reg });
      } catch (err) {
        if (reg !== 'all') {
          data = await fetchAram({ platform: plat, rank: rk, regionOverride: 'all' });
...
module.exports = { getAram, PLATFORM_REGION };
